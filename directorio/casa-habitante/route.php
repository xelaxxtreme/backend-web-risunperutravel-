...
if ($method === 'GET') {
    try {
        $sql = "SELECT * FROM casas_habitantes";
        ...
        echo json_encode(["error" => "Error al obtener casas habitantes"]);
    }
    exit;
}
...
if ($method === 'POST') {
    try {
        ...
        $stmt = $conn->prepare("INSERT INTO casas_habitantes (nombre, numero, lugar, detalles, creado) VALUES (?, ?, ?, ?, ?)");
        ...
        http_response_code(500);
        echo json_encode(["error" => "Error al crear casas habitantes"]);
    }
    exit;
}
