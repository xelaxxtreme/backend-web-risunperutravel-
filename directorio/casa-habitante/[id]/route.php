...
try {
    switch ($method) {
        // ✅ GET → obtener casa_habitante por ID
        case 'GET':
            if (!$id) {
            ...
            $stmt = $conn->prepare("SELECT * FROM casas_habitantes WHERE id = ?");
            ...
            if ($result->num_rows === 0) {
                ...
                echo json_encode(['error' => 'casa_habitante no encontrado']);
                exit;
            }
        ...
        case 'POST':
            ...
            if (isset($data['_method']) && $data['_method'] === 'PUT') {
                ...
                echo json_encode(['message' => 'casa_habitante actualizado correctamente', 'id' => $id]);
            }
            break;
        // ✅ DELETE → eliminar casa_habitante
        case 'DELETE':
            if (!$id) {
                http_response_code(400);
                echo json_encode(['error' => 'Falta el parámetro ID']);
            ...
            $stmt = $conn->prepare("DELETE FROM casas_habitantes WHERE id = ?");
            $stmt->bind_param("i", $id);
            $stmt->execute();

            echo json_encode(['message' => 'casa_habitante eliminado', 'id' => $id]);
            break;
