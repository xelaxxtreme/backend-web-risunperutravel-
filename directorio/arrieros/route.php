...
if ($method === 'GET') {
    try {
        ...
        echo json_encode($arrieros);
    } catch (Exception $e) {
        http_response_code(500);
        echo json_encode(["error" => "Error al obtener arrieros"]);
    }
    exit;
}
...
if ($method === 'POST') {
    try {
        ...
        $stmt = $conn->prepare("INSERT INTO arrieros (nombre, numero, lugar, detalles, creado) VALUES (?, ?, ?, ?, ?)");
        ...
        http_response_code(500);
        echo json_encode(["error" => "Error al crear arriero"]);
    }
    exit;
}
