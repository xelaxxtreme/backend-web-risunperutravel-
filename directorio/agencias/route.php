...
if ($method === 'GET') {
    try {
        ...
        echo json_encode(["error" => "Error al obtener agencias"]);
    }
    exit;
}
...
if ($method === 'POST') {
    try {
        ...
        $stmt = $conn->prepare("INSERT INTO agencias (nombre, numero, lugar, detalles, creado) VALUES (?, ?, ?, ?, ?)");
        ...
        http_response_code(500);
        echo json_encode(["error" => "Error al crear hotel"]);
    }
    exit;
}
