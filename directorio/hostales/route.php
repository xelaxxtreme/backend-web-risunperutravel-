...
if ($method === 'GET') {
    try {
        ...
        echo json_encode($hostales);
    } catch (Exception $e) {
        http_response_code(500);
        echo json_encode(["error" => "Error al obtener hostales"]);
    }
    exit;
}
...
if ($method === 'POST') {
    try {
        ...
        $stmt = $conn->prepare("INSERT INTO hostales (nombre, numero, lugar, detalles, creado) VALUES (?, ?, ?, ?, ?)");
        ...
        http_response_code(500);
        echo json_encode(["error" => "Error al crear hostal"]);
    }
    exit;
}
