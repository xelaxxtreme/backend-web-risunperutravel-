...
try {
    switch ($method) {
        ...
        case 'GET':
            ...
            $stmt = $conn->prepare("SELECT * FROM hostales WHERE id = ?");
            ...
            if ($result->num_rows === 0) {
                ...
                echo json_encode(['error' => 'hostal no encontrado']);
                exit;
            }
        ...
        case 'POST':
            ...
            if (isset($data['_method']) && $data['_method'] === 'PUT') {
                ...
                echo json_encode(['message' => 'hostal actualizado correctamente', 'id' => $id]);
            }
            break;
        // ✅ DELETE → eliminar hostal
        case 'DELETE':
            if (!$id) {
                http_response_code(400);
                echo json_encode(['error' => 'Falta el parámetro ID']);
            ...
            $stmt = $conn->prepare("DELETE FROM hostales WHERE id = ?");
            $stmt->bind_param("i", $id);
            $stmt->execute();

            echo json_encode(['message' => 'hostal eliminado', 'id' => $id]);
            break;
