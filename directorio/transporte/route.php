...
if ($method === 'GET') {
    try {
        ...
        echo json_encode(["error" => "Error al obtener transporte"]);
    }
    exit;
}
...
if ($method === 'POST') {
    try {
        ...
        $stmt = $conn->prepare("INSERT INTO transporte (nombre, numero, lugar, detalles, creado) VALUES (?, ?, ?, ?, ?)");
        ...
        http_response_code(500);
        echo json_encode(["error" => "Error al crear transporte"]);
    }
    exit;
}
