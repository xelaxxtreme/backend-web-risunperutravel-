...
$uploadsPath = __DIR__ . "/../../../recursospagina/imagenes/";
$docsPath = __DIR__ . "/../../../recursospagina/documentos/";
...
if ($method === 'POST' && ($_POST['_method'] ?? '') === 'PUT') {
    try {
        ...
        // Campos de texto
        $fields = [
            'nombre', 'slug', 'idioma', 'descripcion', 'precio', 'precio_enganche', 'duracion',
            'itinerario', 'departamento', 'incluye', 'no_incluye', 'recomendaciones', 'mapa',
            'frase_seo', 'tipo', 'video', 'picos', 'dificultad','idTipo'
        ];
        $values = [];
        foreach ($fields as $f) {
            $values[$f] = $_POST[$f] ?? '';
        ...
        $stmt = $conn->prepare("
            UPDATE tours SET 
                nombre=?, slug=?, idioma=?, descripcion=?, precio=?, precio_enganche=?, duracion=?, itinerario=?, 
                ...
                doc_itinerario=COALESCE(?, doc_itinerario), idTipo=?
            WHERE id=?
        ");
        $stmt->bind_param(
            "sssssdsssssssssssssssi",
            $values['nombre'], $values['slug'], $values['idioma'], $values['descripcion'], $values['precio'],
            $values['precio_enganche'], $values['duracion'], $values['itinerario'], $values['departamento'],
            $values['incluye'], $values['no_incluye'], $values['recomendaciones'], $values['mapa'],
            $imgPortada, $values['frase_seo'], $values['tipo'], $values['video'], $values['picos'],
            $values['dificultad'], $docItinerario, $values['idTipo'], $id
        );
        $stmt->execute();
